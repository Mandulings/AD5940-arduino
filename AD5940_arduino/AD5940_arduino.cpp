...
static AppSWVCfg_Type AppSWVCfg = {
  ...
  32000.0,        // LFOSCClkFreq
  16000000.0,     // SysClkFreq
  16000000.0,     // AdcClkFreq
  10000.0,        // RcalVal
  1820.0f,        // ADCRefVolt

  -1000.0f,       // RampStartVolt
  1000.0f,        // RampPeakVolt
  2200.0f,        // VzeroStart
  400.0f,         // VzeroPeak
  866,            // StepNumber

  1.0f,           // SampleDelay
  LPTIARTIA_20K,  // LPTIARtiaSel
  20000.0f,       // ExternalRtiaValue
  ADCPGA_1,       // AdcPgaGain
  ADCSINC3OSR_4,  // ADCSinc3Osr
  4,              // FifoThresh

  bFALSE,         // SWVInited
  {0},            // RtiaValue (fImpPol_Type)
  {0},            // InitSeqInfo (SEQInfo_Type)
  {0},            // ADCSeqInfo
  bTRUE,          // bFirstDACSeq
  {0},            // DACSeqInfo
  0,              // CurrStepPos
  ...
  25.0f,          // Frequency
  25.0f,          // SqrWvAmplitude
  5.0f,           // SqrWvRampIncrement
  0,              // CurrVzeroCode
  bFALSE,         // bDACCodeInc
  bFALSE,         // bSqrWaveHiLevel
  bFALSE,         // bRampOneDir
  bFALSE,         // StopRequired
  SWV_STATE0      // RampState
};
...
/**
 * @brief Flag to indicate MCU interrupt occurred
 * @note MODIFIED (ADDED from ADICUP3029Port.c)
 */

volatile int ucInterrupted = true;  
 ...
 * @note MODIFIED (Arduino)
 */
void AD5940_Delay10us(uint32_t time){ delayMicroseconds(10 * time); };

/**
 * PIN & SPI_CLOCK initial values
 */
int RESET_PIN = -1;
int CS_PIN = -1;
...
/**
 * Chip Select Clear / Set
 * @note MODIFIED (ADDED)
 */
void AD5940_CsClr() { digitalWrite(CS_PIN, LOW); }
void AD5940_CsSet() { digitalWrite(CS_PIN, HIGH); }
...
/**
 * @brief Get sequencer command63 generated.
...
void AD5940_FIFORd(uint32_t *pBuffer, uint32_t uiReadCount)   
{
  /* Use function AD5940_SPIReadReg to read REG_AFE_DATAFIFORD is also one method. */
   ...
   if(uiReadCount < 3)
   {
      ...
      {
         pBuffer[i] = AD5940_ReadReg(REG_AFE_DATAFIFORD);
      }
   }
   else
   {
      AD5940_CsClr();
      SPI.transfer(SPICMD_READFIFO);
      ...
      {
         uint16_t upper_FIFO = SPI.transfer16(0);
         uint16_t lower_FIFO = SPI.transfer16(0);
         pBuffer[i] = ((uint32_t)upper_FIFO << 16) | lower_FIFO;
         
      }
      /* Read back last two FIFO data with none-zero offset*/
      uint16_t upper_FIFO_2 = SPI.transfer16(0x44444444);
      uint16_t lower_FIFO_2 = SPI.transfer16(0x44444444);
      pBuffer[i++] = ((uint32_t)upper_FIFO_2 << 16) | lower_FIFO_2;
      uint16_t upper_FIFO_3 = SPI.transfer16(0x44444444);
      uint16_t lower_FIFO_3 = SPI.transfer16(0x44444444);
      pBuffer[i] = ((uint32_t)upper_FIFO_3 << 16) | lower_FIFO_3;
      AD5940_CsSet();
   }
}
...
**/
  ...
  void AD5940_SPIWriteReg(uint16_t addr, uint32_t data) {
    AD5940_CsClr();
    ...
    if(((addr>=0x1000)&&(addr<=0x3014))) {
          SPI.transfer16((data >> 16) & 0xFFFF);      // upper 16 bits
          SPI.transfer16(data & 0xFFFF);         // upper 16 bits
      } else {
 ...
 * @return Return register value.
**/
uint32_t AD5940_SPIReadReg(uint16_t addr) {
  uint32_t Data = 0;
  ...
  SPI.transfer(0);  // dummy write to clock out first data

  if ((addr >= 0x1000) && (addr <= 0x3014)) { // read 32 bit
      uint16_t upper = SPI.transfer16(0);
      uint16_t lower = SPI.transfer16(0);
      Data = ((uint32_t)upper << 16) | lower;
  } else { // read 16 bit
...
void AD5940_Initialize(void)
{
  ...
  // Serial.print("ChipID:"); Serial.println(i, HEX);
...
void AD5940_HWReset() {
  ...
  delay(5000);
  Serial.println("Hardware Reseted");
}
...
*/
uint32_t __AD5940_TakeMeasurement(int32_t *time_out)
{
  uint32_t ADCCode = 0;
...
AD5940Err AD5940_LPRtiaCal(LPRTIACal_Type *pCalCfg, void *pResult)
{
  ...
  */
  ...
  if(bDCMode) // Calibrate with DC Voltage
  {
  ...
  else // Calibrate with AC voltage
 {
		hs_loop.WgCfg.SinCfg.SinAmplitudeWord = WgAmpWord;
		hs_loop.WgCfg.SinCfg.SinFreqWord = AD5940_WGFreqWordCal(pCalCfg->fFreq, pCalCfg->SysClkFreq);
		hs_loop.WgCfg.SinCfg.SinOffsetWord = 0;
    ...
    // Serial.print("AFE Measurement2: "); Serial.println(DftRtia.Real);
...
AD5940Err AD5940_LFOSCMeasure(LFOSCMeasure_Type *pCfg, float *pFreq) /* Measure current LFOSC frequency. */
{
   ...
   * **/
   ...
   /**
    * SeqA - SeqB : Wake-up timer : LFOSC
    * SeqBB : MCU clock (16MHz)
    Compare two period to measure the LFOSC Frequency
    */
  ...
  AD5940_SEQGetCfg(&seq_cfg_backup);    // Read current sequencer configuration
  INTCCfg = AD5940_INTCGetCfg(AFEINTC_1);       // Read current Interrupt configuration
  AD5940_INTCCfg(AFEINTC_1, AFEINTSRC_ENDSEQ, bTRUE); // INTC1 configure endseq
	AD5940_INTCClrFlag(AFEINTSRC_ALLINT); // All interrupt clear
  ...
  while(AD5940_INTCTestFlag(AFEINTC_1, AFEINTSRC_ENDSEQ) == bFALSE); // The code stops here sometimes
...
static AD5940Err AppSWVSeqInitGen(void)
{
  ...
  /* Configure Low Power Loop - LPDAC & LPAMP(TIA) */
  ...
  /* Configure Digital Signal Processor - ADC, DFT */
  ...
  error = AD5940_SEQGenFetchSeq(&pSeqCmd, &SeqLen); /* Get the generated sequence */
  ...
  if(error == AD5940ERR_OK)
  {
    ...
    if(SeqLen >= AppSWVCfg.MaxSeqLen)
      ...
      /* Congifure AppSWV with new generated sequence */
...
static AD5940Err AppSWVSeqADCCtrlGen(void)
{
  ...
  AD5940_ClksCalculate(&clks_cal, &WaitClks); /* Calculate how many clocks are needed in sequencer wait command to generate required number of data from filter output. */
...
static AD5940Err RampDacRegUpdate(uint32_t *pDACData)
{
  ...
  else
  {
    ...
    case SWV_STATE2:
      ...
      {
        AppSWVCfg.RampState = SWV_STATE3;   /* Enter State3 */
    ...
    case SWV_STATE3:
      ...
      {
        AppSWVCfg.RampState = SWV_STATE4;   /* Enter State4 */
        AppSWVCfg.CurrVzeroCode = (uint32_t)((AppSWVCfg.VzeroPeak - 200.0f)/DAC6BITVOLT_1LSB);
      }
      break;    
    case SWV_STATE4:
...
#define CURRBLK_BLK1      1   /* Current block is BLOCK1 */
  ...
  {
    Serial.println("Error: Selected Increment, StartVolt and PeakVolt exceed accepted limits"); // MODIFIED (PRINT)
    while(1){}
  }
  ...
  if(AppSWVCfg.bFirstDACSeq == bTRUE)
  {
    ...
    Serial.print("Total Steps: "); Serial.println(AppSWVCfg.StepNumber);
    // Serial.print("Steps/Block: "); Serial.println(StepsPerBlock);
  ...
  // Serial.print("Steps Remainning: "); Serial.println(StepsRemainning);
  if(StepsRemainning == 0) return AD5940ERR_OK; /* Done. */
  bIsFinalBlk = StepsRemainning <= StepsPerBlock?bTRUE:bFALSE;      // Steps remainning this step <= Steps per one block : Final Block
  if(bIsFinalBlk)
...
for(i=0; i<StepsThisBlock - 1; i++)
{
  ...
  SeqCmdBuff[2] = SEQ_WR(bCmdForSeq0?REG_AFE_SEQ1INFO:REG_AFE_SEQ0INFO,\
    (SRAMAddr<<BITP_AFE_SEQ1INFO_ADDR)|(SEQLEN_ONESTEP<<BITP_AFE_SEQ1INFO_LEN)); // Info Cfg for next step
...
static AD5940Err AppSWVRtiaCal(void)
{
  ...
  AD5940_LPRtiaCal(&lprtia_cal, &RtiaCalValue); // Calibration
  AppSWVCfg.RtiaValue = RtiaCalValue;
  // Serial.print("Rtia Magnitude_CAL"); Serial.println(RtiaCalValue.Magnitude);
  // Serial.print("Rtia Phase_CAL"); Serial.println(RtiaCalValue.Phase);
...
AD5940Err AppSWVInit(uint32_t *pBuffer, uint32_t BufferSize)
{
    ...
    (AppSWVCfg.bParaChanged == bTRUE))
  {
    ...
    if(AppSWVCfg.LPTIARtiaSel == LPTIARTIA_OPEN) /* Internal RTIA is opened. User wants to use external RTIA resistor */
    {
      ...
      AppSWVRtiaCal();   // Calibration of Rtias
    
    AppSWVCfg.SWVInited = bFALSE;    
    AD5940_SEQGenInit(pBuffer, BufferSize);
    /* Generate sequence and write them to SRAM start from address AppSWVCfg.SeqStartAddr */
    error = AppSWVSeqInitGen();      /* Application initialization sequence */                      // SEQID_3 : Application Initialization
    if(error != AD5940ERR_OK) return error;
    error = AppSWVSeqADCCtrlGen();   /* ADC control sequence */                                     // SEQID_2 : ADC control sequence
    if(error != AD5940ERR_OK) return error;
  ...
  error = AppSWVSeqDACCtrlGen();                                                            // SEQID_0 : DAC control sequence
  if(error != AD5940ERR_OK) return error;
  ...
  AD5940_SEQMmrTrig(AppSWVCfg.InitSeqInfo.SeqId);                        // Trigger SEQID_3 : Initialize
  while(AD5940_INTCTestFlag(AFEINTC_1, AFEINTSRC_ENDSEQ) == bFALSE);      // Wait for interrupt
  AD5940_INTCClrFlag(AFEINTSRC_ENDSEQ);
  
  AppSWVCfg.ADCSeqInfo.WriteSRAM = bFALSE;
  AD5940_SEQInfoCfg(&AppSWVCfg.ADCSeqInfo);         // SEQID_2
  
  AppSWVCfg.DACSeqInfo.WriteSRAM = bFALSE;
  AD5940_SEQInfoCfg(&AppSWVCfg.DACSeqInfo);         // SEQID_0
  
  AD5940_SEQCtrlS(bFALSE);
  AD5940_WriteReg(REG_AFE_SEQCNT, 0); // Initialize Sequence Count
  ...
  Serial.println("SWV appilcation initialized");
  return AD5940ERR_OK;
}
...
AD5940Err AppSWVISR(void *pBuff, uint32_t *pCount)
{
  ...
  *pCount = 0;
  ...
  if(IntFlag&AFEINTSRC_DATAFIFOTHRESH)
  {
    ...
    Serial.println("Data exceeding the FIFO threshold");
    ...
    AppSWVRegModify((int32_t*)pBuff, &FifoCnt);   // MODIFIED int32_t*
  //  AD5940_SleepKeyCtrlS(SLPKEY_UNLOCK);
    //AD5940_EnterSleepS();
    /* Process data */ 
  ...
  if(IntFlag & AFEINTSRC_ENDSEQ)
  {
    ...
    Serial.print("FIFO Count: "); Serial.println(FifoCnt);
    AD5940_INTCClrFlag(AFEINTSRC_ENDSEQ);
    AD5940_FIFORd((uint32_t *)pBuff, FifoCnt);
    /* Process data */ 
    AppSWVDataProcess((int32_t*)pBuff,&FifoCnt);
...
//   AD5940_Initialize();    /* Call this right after AFE reset */
  ...
  Serial.println("Platform Configuration Done");
...
/**
 * @brief Generate an array of DAC voltage
 * @note MODIFIED (NEW)
 */
void GenerateRampDacCodeArray(float *pDACArray, uint32_t ArraySize)
{
  uint32_t i;

  uint32_t CurrStepPos = 0;
  float CurrRampCode = AppSWVCfg.RampStartVolt/DAC12BITVOLT_1LSB;
  RampState_Type RampState = SWV_STATE0;
  uint32_t CurrVzeroCode = 0;
  BoolFlag bDACCodeInc = AppSWVCfg.bDACCodeInc;
  BoolFlag bSqrWaveHiLevel =  AppSWVCfg.bSqrWaveHiLevel;
  ...
  for(i = 0; i < ArraySize; i++)
  {
    ...
    if (AppSWVCfg.bRampOneDir)
    {
      ...
      {
      case SWV_STATE0:
        CurrVzeroCode = (uint32_t)((AppSWVCfg.VzeroStart - 200.0f)/DAC6BITVOLT_1LSB);
        RampState = SWV_STATE1;
        break;
      case SWV_STATE1:
        if(CurrStepPos >= AppSWVCfg.StepNumber/2)
        {
          RampState = SWV_STATE2;
          CurrVzeroCode = (uint32_t)((AppSWVCfg.VzeroPeak - 200.0f)/DAC6BITVOLT_1LSB);
        }
        break;
      case SWV_STATE2:
        if(CurrStepPos >= AppSWVCfg.StepNumber)
          RampState = SWV_STOP;
        break;
      case SWV_STOP:
        break;
    ...
    else
    {
      ...
      case SWV_STATE0:
        ...
        break;
      case SWV_STATE1:
        if(CurrStepPos >= AppSWVCfg.StepNumber/4)
        {
          RampState = SWV_STATE2;
          CurrVzeroCode = (uint32_t)((AppSWVCfg.VzeroPeak - 200.0f)/DAC6BITVOLT_1LSB);
        }
        break;
      case SWV_STATE2:
        if(CurrStepPos >= (AppSWVCfg.StepNumber*2)/4)
        {
          RampState = SWV_STATE3;
          bDACCodeInc = bDACCodeInc ? bFALSE : bTRUE;
        }
        break;
      case SWV_STATE3:
        if(CurrStepPos >= (AppSWVCfg.StepNumber*3)/4)
        {
          RampState = SWV_STATE4;
          CurrVzeroCode = (uint32_t)((AppSWVCfg.VzeroPeak - 200.0f)/DAC6BITVOLT_1LSB);
        }
        break;
      case SWV_STATE4:
    ...
    {
      if(bDACCodeInc)
        CurrRampCode -= (AppSWVCfg.DACCodePerStep - AppSWVCfg.DACCodePerRamp);
      else
        CurrRampCode -= AppSWVCfg.DACCodePerStep;
    ...
    VzeroCode = CurrVzeroCode;
    VbiasCode = (uint32_t)(VzeroCode*64 + CurrRampCode);
    ...
    if (VbiasCode >= VzeroCode * 64) {
      Vbias_V = 0.2f + (VbiasCode-1) * (0.0005372f);
    } else {
      Vbias_V = 0.2f + (VbiasCode * 0.0005372f);
    }

    Vzero_V = 0.2f + (VzeroCode * 0.03438f);

    float delta_V = Vbias_V - Vzero_V;
...
uint32_t AppSWVGetStepNumber(void)
{
  return AppSWVCfg.StepNumber;
}

float AppSWVGetRampFrequency(void)
{
  return AppSWVCfg.Frequency;
}
